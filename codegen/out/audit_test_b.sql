-- create necessary audit columns and table for test_b
...
-- create audit table:
drop table if exists test_b_audit;
create table test_b_audit as select * from test_b where 1=2;

alter table test_b_audit
...
--
create or replace function test_b_log() returns trigger as $BODY$
  begin
    if (tg_op = 'DELETE') then
      insert into test_b_audit select  OLD.*, 'D', now(), user ;
      return old;
    elsif (tg_op = 'UPDATE') then
      if ( old.id != new.id) then
         raise exception 'Cannot update primary key';
      end if;
      insert into test_b_audit select  OLD.*, 'U', now(), user ;
      return new;    
    end if;
    return null;
  end;
...
create trigger test_b_audit_u after update
on test_b 
for each row 
when (old.* is distinct from new.*)
execute procedure test_b_log();

create trigger test_b_audit_d after delete
on test_b 
for each row 
execute procedure test_b_log();
